...
public class selectComparison_MB implements Serializable {
    ...
    public void processPreferences(String prefId) {
        String[] postArr = postCode1.split(",");
        DbpostCode postObj = new DbpostCode(new DbpostCodePK(Integer.parseInt(postArr[0]), Integer.parseInt(postArr[1])));
        switch (prefId) {
            case "one":
                for (DbpostCode item : listOfPostCode1) {
                    if (!item.equals(postObj) && !listOfPostCode2.contains(postObj)) {
            ...
            case "two":
                for (DbpostCode item : listOfPostCode2) {
                    if (!item.equals(postObj) && !listOfPostCode3.contains(postObj)) {
