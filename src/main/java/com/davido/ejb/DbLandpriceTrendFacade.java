 ...
 */
@Stateless
public class DbLandpriceTrendFacade extends AbstractFacade<DbLandpriceTrend> implements DbLandpriceTrendFacadeLocal {
    ...
    public DbLandpriceTrendFacade() {
        super(DbLandpriceTrend.class);
    }
    ...
    public List<Object[]> findTrends(String postcode, String postCodeLine) {
        ...
        try {
            querySTR = "SELECT a.postCode, a.postLine, a.year, "
                    + "CASE WHEN a.dataType = 'R' THEN ROUND(AVG(a.price)) ELSE 'null' END, "
                    + "CASE WHEN a.dataType = 'P' THEN ROUND(AVG(a.price)) ELSE 'null' END "
                    + "FROM db_landprice_trend a "
                    + "WHERE a.postCode = ?1 "
                    + "AND a.postLine = ?2 "
